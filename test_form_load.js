...
for (let i = 1; i <= NUMBER_OF_REQUESTS; i++) {
    ...
    const row = [
        ...
        mssv.toLowerCase() + "@student.tdtu.edu.vn", // Email Sinh Viên (cột 1)
        mssv,                                 // MSSV (cột 2)
        "Test Sinh Viên " + i,               // Họ tên (cột 3)
        "Tập sự nghề nghiệp",                // Học phần (cột 4)
        "Học Kỳ 2",                          // Học kỳ (cột 5)
        "2026 - 2027",                       // Năm học (cột 6)
        i % 2 === 0 ? "Đã có doanh nghiệp" : "Chưa có doanh nghiệp",  // Trạng thái (cột 7)
        i % 2 === 0 ? "0123456789" + i : "", // Mã số thuế (cột 8)
        i % 2 === 0 ? "Công ty Test " + i : "",  // Tên DN (cột 9)
        "Hà Nội",                            // Địa chỉ (cột 10)
        "test" + i + ".vn",                  // Website (cột 11)
        "hr" + i + "@test.vn"                // Email DN (cột 12)
    ];
    batchData.push(row);
}
